...
class PW:
    def __init__(self, outpath, env, variables={}, calculation="scf"):
        ...
        # create input file
        self.save_in(outpath, env)
        # run command
        self.command = f"pw.x -in {self.calculation}.in | tee {self.calculation}.out"

    def save_in(self, outpath, env):
        ...
        tmp = repr('\n')
        eval(
            f"print(*{self.calculation}_in, sep={tmp}, end={tmp}, file=open('{outpath}/{self.calculation}.in','w'))")
