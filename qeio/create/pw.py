...
def create_pw_in(path, env, calculation="scf", restart_mode="from_scratch"):
    ...
    systems_noend = ["&SYSTEM", "ibrav = 0", f"nat = {env['nat']}", f"ntyp = {env['ntyp']}", f"ecutwfc = {env['ecutwfc']}", f"ecutrho = {env['ecutrho']}", "occupations = 'tetrahedra_opt'"]
    ...
    if calculation == "scf":
        ...
        scf_in = control_noend + ["/"] + systems_noend + spin + soc + hubbard + ["/"] + electrons + ["CELL_PARAMETERS angstrom"] + cell_parameters + ["ATOMIC_SPECIES"] + atomic_species + ["ATOMIC_POSITIONS crystal"] + atomic_positions + kpoints
    elif calculation == "vcrelax":
        conv_thr = ["etot_conv_thr = 1.0e-5", "forc_conv_thr = 1.0e-4", "/"]
        ions = ["&IONS", "ion_dynamics = 'bfgs'", "/"]
        cell = ["&CELL", "cell_dynamics = 'bfgs'", f"press = {env['press']}", "/"]
        kpoints = ["K_POINTS automatic", f"{int(nk[0])} {int(nk[1])} {int(nk[2])} 0 0 0"]
        vcrelax_in = control_noend + conv_thr + systems_noend + spin + soc + hubbard + ["/"] + electrons + ions + cell + ["CELL_PARAMETERS angstrom"] + cell_parameters + ["ATOMIC_SPECIES"] + atomic_species + ["ATOMIC_POSITIONS crystal"] + atomic_positions + kpoints
    elif calculation == "nscf":
        nbnd = [f"nbnd = {env['nbnd']}", "/"]
        kpoints = ["K_POINTS automatic", f"{int(nk[0])*2} {int(nk[1])*2} {int(nk[2])*2} 0 0 0"]
        nscf_in = control_noend + ["/"] + systems_noend + spin + soc + hubbard + nbnd + electrons + ["CELL_PARAMETERS angstrom"] + cell_parameters + ["ATOMIC_SPECIES"] + atomic_species + ["ATOMIC_POSITIONS crystal"] + atomic_positions + kpoints
    elif calculation == "bands":
        nbnd = [f"nbnd = {env['nbnd']}", "/"]
        kpath = ["K_POINTS crystal"] + [f"{env['nks']}"] + [f"{kcoord[0]} {kcoord[1]} {kcoord[2]} 1.0" for kcoord in env['kpath']]
        nscf_in = control_noend + ["/"] + systems_noend + spin + soc + hubbard + nbnd + electrons + ["CELL_PARAMETERS angstrom"] + cell_parameters + ["ATOMIC_SPECIES"] + atomic_species + ["ATOMIC_POSITIONS crystal"] + atomic_positions + kpath
