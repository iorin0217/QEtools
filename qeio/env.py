...
class Env:
    def __init__(self, structure, variables={}, atoms=[], spin_structure={}):
        ...
        self.input_crystal, self.input_atoms, self.input_spin = structure, atoms, spin_structure
        # variables
    ...
    def _set_elements(self):
        # setting
        if not self.input_atoms:
            self.input_atoms = [str(u) for u in self.input_crystal.species]
        ...
        ecutrho = 0
        time_reversal = True
        for atom in set(self.input_atoms):
        ...
        if self.input_spin:
            parallel = False
            for i, j in itertools.permutations(self.input_spin, 2):
                # VESTA VECTR has 1e-5 significatn digits
                parallel = parallel or np.all(
                    np.abs(np.cross(self.input_spin[i]["vec"], self.input_spin[j]["vec"])) < 1e-5)
    ...
    def _set_structure(self):
        ...
        _atomic_numbers = [Element(re.findall(r'(\d+|\D+)', i)[0]).number if len(re.findall(r'(\d+|\D+)', i)) == 1 else Element(
            re.findall(r'(\d+|\D+)', i)[0]).number + 1000 * (int(re.findall(r'(\d+|\D+)', i)[1]) - 1) for i in self.input_atoms]
        skp = seekpath.get_explicit_k_path((self.input_crystal.lattice.matrix, self.input_crystal.frac_coords,
        ...
        crystal = Structure(Lattice(_avec, [re.findall(r'(\d+|\D+)', i)[0] for i in atoms], _pos)
        # map spin structure from input to seekpath basis via spglib
        spin={}
        if self.input_spin:
            spg=spglib.get_symmetry_dataset(
                (self.input_crystal.lattice.matrix, self.input_crystal.frac_coords, _atomic_numbers))
            ...
            mapping=np.linalg.inv(
                skp["primitive_lattice"].T) @ spg["std_rotation_matrix"] @ self.input_crystal.lattice.matrix.T @ permutation
            for atom in self.input_spin:
                vec=skp["primitive_lattice"].T @ mapping @ self.input_spin[atom]["vec"] / \
                    np.linalg.norm(skp["primitive_lattice"], axis=1)
                spin.update({atom: {"frac_coord": [
                                mapping @ frac for frac in self.input_spin[atom]["frac_coord"]], "vec": vec}})
