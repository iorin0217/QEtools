...
def create_env(structure_file, variables, extfields={"press": 0}, constraints={"symm": False}):
    ...
    with open("/home/CMD35/cmd35stud07/QExPy/settings/elements.json", "r") as f:
    ...
    if spin_structure:
        ...
        for i, j in itertools.permutations(spin_structure, 2):
            parallel = parallel or np.all(np.abs(np.cross(spin_structure[i]["vec"],
                                                          spin_structure[j]["vec"])) < 1e-5)  # VESTA VECTR has 1e-5 significatn digits
...
def create_job_sh(path):
    ...
    default = ["mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/pw.x -in scf.in | tee scf.out", "mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/pw.x -in nscf.in | tee nscf.out", "mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/fermi_velocity.x -in nscf.in | tee fermi_velocity.out",
               "mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/projwfc.x -in pdos.in | tee pdos.out", "mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/pw.x -in bands.in | tee bands.out", "mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/bands.x -in band.in", "mpirun -np $NSLOTS /home/CMD35/cmd35stud07/QE6.4.1/bin/projwfc.x -in fatbands.in | tee fatbands.out"]
    job_sh = pbs+default
...
variables = {"reference_distance": 0.025, "dk_grid": 0.2, "occupations": "tetrahedra_opt", "diago_full_acc": ".true.",
             "diagonalization": "david", "mixing_beta": 0.2, "threshold": 1.0e-12, "functional": "PBE", "pseudo_dir": "/home/CMD35/cmd35stud07/QExPy/settings/pseudos", "deltae": 0.01}
# %%
...
# %%
cif = "/home/CMD35/cmd35stud07/experiments/AgCrSe2_afm/AgCrSe2_2.vesta"
env = create_env(cif, variables)
